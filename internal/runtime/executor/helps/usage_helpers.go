...
func ParseClaudeUsage(data []byte) usage.Detail {
	...
	// Estimate reasoning tokens from thinking content blocks. Claude API does not
	// expose thinking tokens in the usage object, so we approximate by summing
	// thinking block text lengths / 4.
...
func (a *ClaudeStreamUsageAccumulator) ProcessLine(line []byte) {
	...
	a.sawUsage = true
	mergeClaudeUsageDetail(&a.detail, parseClaudeUsageNode(usageNode))
}
...
func mergeClaudeUsageDetail(dst *usage.Detail, detail usage.Detail) {
	if dst == nil {
		return
	}
	if detail.InputTokens > 0 {
		dst.InputTokens = detail.InputTokens
	}
	if detail.OutputTokens > 0 {
		dst.OutputTokens = detail.OutputTokens
	}
	if detail.CacheReadTokens > 0 {
		dst.CacheReadTokens = detail.CacheReadTokens
		dst.CachedTokens = detail.CacheReadTokens
	}
	if detail.CacheCreationTokens > 0 {
		dst.CacheCreationTokens = detail.CacheCreationTokens
		if dst.CachedTokens == 0 {
			dst.CachedTokens = detail.CacheCreationTokens
		}
	}
}
