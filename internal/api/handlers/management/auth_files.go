...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	// Fall back to Metadata for auths registered via UploadAuthFile (no synthesizer).
	if enabled, ok := authEnable1MContextValue(auth); ok && enabled {
...
func authEnable1MContextValue(auth *coreauth.Auth) (bool, bool) {
	if auth == nil {
		return false, false
	}
	if auth.Attributes != nil {
		if raw := strings.TrimSpace(auth.Attributes["enable_1m_context"]); raw != "" {
			parsed, errParse := strconv.ParseBool(raw)
			if errParse == nil {
				return parsed, true
			}
	...
	return authFileBoolValue(auth.Metadata["enable_1m_context"])
}
...
func syncAuthFileMetadataFields(auth *coreauth.Auth, touchedRoots map[string]struct{}) {
	...
	if _, ok := touchedRoots["enable_1m_context"]; ok {
		syncAuthFileEnable1MContextAttribute(auth)
	}
...
func syncAuthFileEnable1MContextAttribute(auth *coreauth.Auth) {
	if auth == nil {
		return
	}
	if auth.Attributes == nil {
	...
	enabled, ok := authFileBoolValue(auth.Metadata["enable_1m_context"])
	if !ok || !enabled {
		delete(auth.Metadata, "enable_1m_context")
		delete(auth.Attributes, "enable_1m_context")
		return
	}
	auth.Metadata["enable_1m_context"] = true
	auth.Attributes["enable_1m_context"] = "true"
}
