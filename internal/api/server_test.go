...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	if legacyRR.Code != http.StatusOK {
		t.Fatalf("usage snapshot status = %d, want %d body=%s", legacyRR.Code, http.StatusOK, legacyRR.Body.String())
	}
	if !strings.Contains(legacyRR.Body.String(), `"usage"`) {
		t.Fatalf("usage snapshot body = %s, want usage field", legacyRR.Body.String())
	}
